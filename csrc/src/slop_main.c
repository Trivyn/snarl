                    ...
                    if (((i + 1) < argc)) {
                        {
                            ...
                            __auto_type _mv_252 = strlib_parse_int(val_str);
                            if (_mv_252.is_ok) {
...
slop_option_index_IndexedGraph main_load_graph(slop_arena* arena, slop_string path, uint8_t quiet) {
    __auto_type _mv_253 = ttl_parse_ttl_file(arena, path);
    if (!_mv_253.is_ok) {
        __auto_type e = _mv_253.data.err;
        printf("%s", "Error: failed to parse ");
        printf("%.*s\n", (int)(path).len, (path).data);
        __auto_type _mv_254 = e;
        switch (_mv_254.tag) {
            case ttl_TtlFileError_parse_error:
            {
                __auto_type pe = _mv_254.data.parse_error;
                printf("%s", "  at line ");
            ...
            {
                __auto_type _ = _mv_254.data.file_error;
    ...
    uint8_t** argv = (uint8_t**)_c_argv;
    {
        ...
        slop_arena* arena = &_arena;
        {
            ...
            if (args.show_version) {
                ...
                __auto_type _mv_255 = args.data_file;
                if (!_mv_255.has_value) {
                    ...
                    __auto_type data_path = _mv_255.value;
                    if (args.show_help) {
                        main_print_usage();
                        return 0;
                    } else {
                        {
                            ...
                            __auto_type _mv_256 = main_load_graph(arena, data_path, quiet);
                            if (!_mv_256.has_value) {
                                return 2;
                            } else if (_mv_256.has_value) {
                                __auto_type data_graph = _mv_256.value;
                                {
                                    __auto_type shapes_path = ({ __auto_type _mv = args.shapes_file; _mv.has_value ? ({ __auto_type sp = _mv.value; sp; }) : (data_path); });
                                    {
                                        ...
                                        __auto_type _mv_257 = shapes_result;
                                        if (!_mv_257.has_value) {
                                            return 2;
                                        } else if (_mv_257.has_value) {
                                            __auto_type shapes_graph = _mv_257.value;
                                            {
                                                ...
                                                __auto_type _mv_258 = snarl_validate_with_config(arena, data_graph, shapes_graph, config);
                                                switch (_mv_258.tag) {
                                                    case types_ValidatorResult_validate_success:
                                                    {
                                                        __auto_type report = _mv_258.data.validate_success;
                                                        {
                                                            ...
                                                            __auto_type _mv_259 = args.emit_file;
                                                            if (_mv_259.has_value) {
                                                                __auto_type emit_path = _mv_259.value;
                                                                if (!(quiet)) {
                                                    ...
                                                    {
                                                        __auto_type msg = _mv_258.data.validate_error;
                                                        printf("%s", "Error: ");

...
slop_option_index_IndexedGraph test_cli_load_test_graph(slop_arena* arena, slop_string path) {
    __auto_type _mv_260 = ttl_parse_ttl_file(arena, path);
    if (_mv_260.is_ok) {
        __auto_type g = _mv_260.data.ok;
        {
...
uint8_t test_cli_assert_conforms(slop_arena* arena, slop_string path) {
    __auto_type _mv_261 = test_cli_load_test_graph(arena, path);
...
uint8_t test_cli_assert_violations(slop_arena* arena, slop_string path, int64_t expected_count) {
    __auto_type _mv_262 = test_cli_load_test_graph(arena, path);
    if (_mv_262.has_value) {
        __auto_type g = _mv_262.value;
        __auto_type _mv_263 = snarl_validate(arena, g, g);
        switch (_mv_263.tag) {
            case types_ValidatorResult_validate_success:
            {
                __auto_type report = _mv_263.data.validate_success;
                {
            ...
            {
                __auto_type msg = _mv_263.data.validate_error;
...
uint8_t test_cli_assert_has_violations(slop_arena* arena, slop_string path) {
    __auto_type _mv_264 = test_cli_load_test_graph(arena, path);
    if (_mv_264.has_value) {
        __auto_type g = _mv_264.value;
        {
            __auto_type result = snarl_conforms(arena, g, g);
            if (!(result)) {
                printf("%s", "  PASS: ");
...
uint8_t test_cli_assert_conforms_separate(slop_arena* arena, slop_string data_path, slop_string shapes_path) {
    __auto_type _mv_265 = test_cli_load_test_graph(arena, data_path);
    if (_mv_265.has_value) {
        __auto_type dg = _mv_265.value;
        __auto_type _mv_266 = test_cli_load_test_graph(arena, shapes_path);
...
uint8_t test_cli_assert_violations_separate(slop_arena* arena, slop_string data_path, slop_string shapes_path, int64_t expected_count) {
    __auto_type _mv_267 = test_cli_load_test_graph(arena, data_path);
    if (_mv_267.has_value) {
        __auto_type dg = _mv_267.value;
        __auto_type _mv_268 = test_cli_load_test_graph(arena, shapes_path);
        if (_mv_268.has_value) {
            __auto_type sg = _mv_268.value;
            __auto_type _mv_269 = snarl_validate(arena, dg, sg);
            switch (_mv_269.tag) {
                case types_ValidatorResult_validate_success:
                {
                    __auto_type report = _mv_269.data.validate_success;
                    {
                        __auto_type violations = snarl_get_violations(arena, report);
                        __auto_type actual_count = ((int64_t)((violations).len));
                        if (actual_count == expected_count) {
                ...
                case types_ValidatorResult_validate_error:
                {
                    __auto_type msg = _mv_269.data.validate_error;
                    printf("%s", "  FAIL: validation error: ");
                    printf("%.*s\n", (int)(msg).len, (msg).data);
                    return 0;
                }
...
uint8_t test_cli_test_rdf12_annotation_block(slop_arena* arena) {
    __auto_type _mv_270 = test_cli_load_test_graph(arena, SLOP_STR("fixtures/rdf12-annotation-valid.ttl"));
    if (_mv_270.has_value) {
        __auto_type g = _mv_270.value;
        {
...
uint8_t test_cli_test_report_string(slop_arena* arena) {
    __auto_type _mv_271 = test_cli_load_test_graph(arena, SLOP_STR("fixtures/cardinality-violation.ttl"));
    if (_mv_271.has_value) {
        __auto_type g = _mv_271.value;
        __auto_type _mv_272 = snarl_validate(arena, g, g);
        switch (_mv_272.tag) {
            case types_ValidatorResult_validate_success:
            {
                __auto_type report = _mv_272.data.validate_success;
                {
            ...
            case types_ValidatorResult_validate_error:
            {
                __auto_type _ = _mv_272.data.validate_error;

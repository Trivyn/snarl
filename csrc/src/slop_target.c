...
target_TermList target_resolve_target_class_cached(slop_arena* arena, data_graph_SnarlDataGraph data_graph, target_TargetCache cache, rdf_Term class_term) {
    ...
    __auto_type _mv_123 = ({ void* _ptr = slop_map_get(cache.target_classes, &(class_term)); _ptr ? (slop_option_target_TermList){ .has_value = true, .value = *(target_TermList*)_ptr } : (slop_option_target_TermList){ .has_value = false }; });
    if (_mv_123.has_value) {
        __auto_type cached = _mv_123.value;
        return cached;
...
uint8_t target_class_index_has_instance(target_ClassIndex index, rdf_Term node, rdf_Term class_term) {
    ...
    if (_mv_129.has_value) {
        __auto_type instances = _mv_129.value;
...
slop_list_rdf_Term target_find_subclasses(slop_arena* arena, data_graph_SnarlDataGraph data_graph, rdf_Term class_term) {
    slop_list_rdf_Term _retval = {0};
    {
        ...
        while (qi < ((int64_t)((queue).len))) {
            __auto_type _mv_130 = ({ __auto_type _lst = queue; size_t _idx = (size_t)qi; slop_option_rdf_Term _r = {0}; if (_idx < _lst.len) { _r.has_value = true; _r.value = _lst.data[_idx]; } else { _r.has_value = false; } _r; });
            if (_mv_130.has_value) {
                __auto_type current = _mv_130.value;

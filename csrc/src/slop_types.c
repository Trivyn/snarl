...
types_ValidationReport types_make_validation_report(slop_arena* arena) {
    types_ValidationReport _retval;
...
types_ValidationReport types_report_add_result(slop_arena* arena, types_ValidationReport report, types_ValidationResult result) {
    ...
    {
        __auto_type still_conforms = ((({ __auto_type _mv = result.severity; uint8_t _mr = {0}; switch (_mv) { case types_Severity_severity_violation: { _mr = 1; break; } default: { _mr = 0; break; }  } _mr; })) ? 0 : report.conforms);

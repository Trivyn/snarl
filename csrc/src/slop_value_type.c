...
data_graph_SnarlDataGraph value_type_fixture_g_focus_is_person(slop_arena* arena);
data_graph_SnarlDataGraph value_type_fixture_g_empty(slop_arena* arena);
slop_option_types_ValidationResult snarl_check_class(slop_arena* arena, data_graph_SnarlDataGraph data_graph, rdf_Term focus_node, rdf_Term value_node, rdf_Term required_class, slop_option_types_ShaclPath path, rdf_Term shape_id, types_Severity severity, slop_option_string message);
...
data_graph_SnarlDataGraph value_type_fixture_g_focus_is_person(slop_arena* arena) {
    {
        ...
        g = snarl_data_graph_add(arena, g, rdf_make_triple(arena, rdf_make_iri(arena, value_type_EX_FOCUS), rdf_make_iri(arena, vocab_RDF_TYPE), rdf_make_iri(arena, value_type_EX_PERSON)));
        return g;
    }
}

data_graph_SnarlDataGraph value_type_fixture_g_empty(slop_arena* arena) {
    return snarl_data_graph_create(arena);
}
    ...
    switch (_mv_138.tag) {
        case rdf_Term_term_literal:
        {
            __auto_type lit = _mv_138.data.term_literal;
            {
                ...
                __auto_type _mv_139 = lit.datatype;
                if (_mv_139.has_value) {
                    __auto_type dt = _mv_139.value;
...
uint8_t value_type_node_kind_matches(types_NodeKind required, rdf_TermKind kind) {
    __auto_type _mv_140 = required;
    switch (_mv_140) {
        case types_NodeKind_node_kind_blank_node: {
            __auto_type _mv_141 = kind;
            switch (_mv_141) {

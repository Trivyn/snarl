...
slop_string string_term_string_value(rdf_Term t) {
    __auto_type _mv_92 = t;
    switch (_mv_92.tag) {
        case rdf_Term_term_literal:
        {
            __auto_type lit = _mv_92.data.term_literal;
            return lit.value;
        }
        case rdf_Term_term_iri:
        {
            __auto_type iri = _mv_92.data.term_iri;
            return iri.value;
        }
        case rdf_Term_term_blank:
        {
            __auto_type _ = _mv_92.data.term_blank;
            return SLOP_STR("");
        }
    }
}
...
slop_string string_term_lang(rdf_Term t) {
    __auto_type _mv_93 = t;
    switch (_mv_93.tag) {
        case rdf_Term_term_literal:
        {
            __auto_type lit = _mv_93.data.term_literal;
            __auto_type _mv_94 = lit.lang;
            if (_mv_94.has_value) {
                __auto_type lang = _mv_94.value;
                return lang;
...
uint8_t string_term_is_literal(rdf_Term t) {
    __auto_type _mv_95 = t;
    switch (_mv_95.tag) {
        case rdf_Term_term_literal:
        {
            __auto_type _ = _mv_95.data.term_literal;
            return 1;
        }
        default: {
            return 0;
...
slop_option_types_ValidationResult snarl_check_language_in(slop_arena* arena, rdf_Term focus_node, rdf_Term value_node, slop_list_string allowed_langs, slop_option_types_ShaclPath path, rdf_Term shape_id, types_Severity severity, slop_option_string message) {
    ...
    slop_option_types_ValidationResult _retval;
    __auto_type _mv_96 = value_node;
    switch (_mv_96.tag) {
        case rdf_Term_term_literal:
        {
            __auto_type lit = _mv_96.data.term_literal;
            __auto_type _mv_97 = lit.lang;
            if (_mv_97.has_value) {
                __auto_type lang = _mv_97.value;
                {
                    __auto_type found = 0;
                    {
...
slop_list_types_ValidationResult snarl_check_unique_lang(slop_arena* arena, rdf_Term focus_node, slop_list_rdf_Term value_nodes, slop_option_types_ShaclPath path, rdf_Term shape_id, types_Severity severity, slop_option_string message) {
    slop_list_types_ValidationResult _retval;
    {
        ...
        __auto_type seen = slop_map_new_ptr(arena, 16, sizeof(slop_string), slop_hash_string, slop_eq_string);
        {
            ...
            for (size_t _i = 0; _i < _coll.len; _i++) {
                ...
                __auto_type _mv_98 = vn;
                switch (_mv_98.tag) {
                    case rdf_Term_term_literal:
                    {
                        __auto_type lit = _mv_98.data.term_literal;
                        __auto_type _mv_99 = lit.lang;
                        if (_mv_99.has_value) {
                            __auto_type lang = _mv_99.value;
